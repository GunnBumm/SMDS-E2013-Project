	...
	public void receive(Message msg) {
		...
		if (DeserializeEnvelope.command.equals("add")) {
			...
			if (taskWithId == null) {
				...
				provider.TaskManagerInstance.tasks.add(DeserializeEnvelope.data
						.get(0));
				...
				System.out.println(prefix + "Task with Id:"
						+ DeserializeEnvelope.data.get(0).id + " added! "
						+ "total number of tasks: "
						+ provider.TaskManagerInstance.tasks.size());
			} else {
				System.out.println(prefix + "Task with Id:"
						+ DeserializeEnvelope.data.get(0).id
						+ " already exists and hence NOT added! "
						+ "Total number of tasks: "
						+ provider.TaskManagerInstance.tasks.size());
			}
			...
			if (taskWithId != null) {
				...
				System.out.println(prefix + "Task with Id:"
						+ DeserializeEnvelope.data.get(0).id
						+ " can not be found and hence NOT deleted!"
						+ " Total number of tasks: "
						+ provider.TaskManagerInstance.tasks.size());
			}
			...
			provider.TaskManagerInstance.tasks
					.removeAll(provider.TaskManagerInstance.tasks);

			try {
				provider.PersistTaskManager();
			} catch (JAXBException ex) {
				System.out.println(prefix
						+ "Failed to serialize task manager. Error message:"
						+ ex.getMessage());
			} catch (IOException ex) {
				System.out.println(prefix
						+ "Failed to save task manager. Error message:"
						+ ex.getMessage());
			}
			...
			String taskId = DeserializeEnvelope.id;
			if (GetTaskWithId(taskId) != null) {
				for (int index = 0; index < provider.TaskManagerInstance.tasks
						.size(); index++) {
					if (provider.TaskManagerInstance.tasks.get(index).id
							.equals(taskId)) {
			...
			for (int index = 0; index < provider.TaskManagerInstance.tasks
					.size(); index++) {
				if (provider.TaskManagerInstance.tasks.get(index).role
						.equals(DeserializeEnvelope.role)) {
					System.out.println("ID: "
							+ provider.TaskManagerInstance.tasks.get(index).id);
					System.out
							.println("Name: "
									+ provider.TaskManagerInstance.tasks
											.get(index).name);
